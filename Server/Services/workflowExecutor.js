...
const executeNode = async (node, nextNode) => {
  if (node.type === 'coldEmail') {
    await agenda.now('coldEmail', { email: node.data.email, subject: node.data.subject, body: node.data.body , workflowId: node.workflowId });
    ...
    if (nextNode) {
      ...
      const nextJobData = { ...nextNode.data, workflowId: node.workflowId };
    ...
    await agenda.now('lead source', { source: node.data.source, workflowId: node.workflowId  });
    return;
  }
};
